                ...
                <div class="table-responsive">
                    ...
                    <div class="row" style="padding-top: 12px;padding-bottom: 12px;background-color: #eff3f8;margin-right: 0px;margin-left: 0px;">
                        <form name="search" action="" method="get">
                            ...
                            <div class="col-sm-3">
                                ...
                                <label>轮播名称： <input type="text" name="name" value="" /></label>
                    ...
                    <table id="sample-table-1" class="table table-striped table-bordered table-hover" cellpadding=0 cellspacing=0 >
                        ...
                        <tr>
                            ...
                            <th><a href="javascript:sortBy('title','1','index')" title="按照文章标题升序排列 ">信息名称</a></th>
                            <th width="80px"><a href="javascript:sortBy('sort','1','index')" title="按照排序升序排列 ">排序<img src="{{ asset('admin_assets/images/desc.gif') }}" width="12" height="17" border="0" align="absmiddle"></a></th>
                            <th width="80px"><a href="javascript:sortBy('view','1','index')" title="按照浏览次数升序排列 ">链接地址</a></th>
                            <th><a href="javascript:sortBy('create_time','1','index')" title="按照发表时间升序排列 ">发表时间</a></th>
                            <th >操作</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($banners as $key=>$val)
                            <tr>
                                ...
                                <td>{{ $val->id }}</td>
                                <td><a href="{{ url('newwebadmin/banner/'.$val->id.'/edit') }}">{{ $val->name }}</a></td>
                                ...
                                <td>
                                    <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                        <a class="btn btn-primary btn-xs" href="{{ url('newwebadmin/banner/'.$val->id.'/edit') }}"><i class="icon-edit">编辑</i></a>&nbsp;
    ...
    <script>
        function del(id){
            layer.confirm('您确定要删除吗？', {
                ...
                //异步删除
                $.post('{{ url('newwebadmin/banner/') }}/'+id,{'_method':'delete','_token':_token},function(data){
                    if(data.status == '1'){
        ...
        function foreverdel(id)
        {
            ...
            layer.confirm('您确定要删除吗？', {
                ...
                var _token = '{{csrf_token()}}';
                //异步删除
                $.post('{{ url('newwebadmin/foreverdel') }}',{'table':'banners',id:id,'_token':_token},function(data){
                    if(data.status == '1'){
                        //$(obj).parent().parent().remove();
                        layer.msg(data.msg, {icon: 1});
                        location.href = location.href;
